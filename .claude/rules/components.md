...
- React island is correct for: the catering form (form state + Turnstile), the franchising form, the catering success state (custom post-submit animation).
- BrandShowcase is `.astro` — scroll-snap is layout, not interactivity.
...
- If a component has more than one variant (e.g. `Button`), use a single component with prop variants — do not proliferate files.
...
- **Headings**: maintain a sensible h1–h6 outline per page. Don't pick heading levels for typographic size — use CSS for size, semantics for hierarchy.
...
- No utility-class framework. The project's aesthetic stance is anti-generic; bespoke CSS gives the right control. If a utility pattern emerges three times, extract a class or a token.
- Use modern CSS: `:has()`, `@layer`, `@scope`, container queries, `light-dark()`, `color-mix()`. Browser baseline is 2024+.
